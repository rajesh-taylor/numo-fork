        ...
        lifecycleScope.launch {
            ...
            try {
                ...
                val historyEntry = autoWithdrawManager.addManualWithdrawalEntry(
                    ...
                    status = io.refueler.merchant.feature.autowithdraw.WithdrawHistoryEntry.STATUS_PENDING,
                    quoteId = quoteId,
                    errorMessage = null
                )
                ...
                withContext(Dispatchers.Main) {
                    ...
                    when (finalized.state) {
                        QuoteState.PAID -> {
                            ...
                            val destinationLabel = lightningAddress ?: request
                            io.refueler.merchant.core.dev.WalletLogger.log("OUT", amount, mintUrl, "Withdrawal successful: $destinationLabel")
                            showPaymentSuccess()
                        }
                        QuoteState.UNPAID -> {
                            withdrawEntryId?.let {
                                autoWithdrawManager.updateWithdrawalStatus(
                                    id = it,
                                    status = io.refueler.merchant.feature.autowithdraw.WithdrawHistoryEntry.STATUS_FAILED,
