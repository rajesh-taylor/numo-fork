                ...
                for (i in 0 until array.length()) {
                    ...
                    val item = Item().apply {
                        ...
                        if (!obj.isNull("priceType")) {
                            priceType = try {
                                io.refueler.merchant.core.model.PriceType.valueOf(obj.getString("priceType"))
                            } catch (e: IllegalArgumentException) {
                                io.refueler.merchant.core.model.PriceType.FIAT
                            }
                        }
    ...
    fun importItemsFromCsv(csvFilePath: String, clearExisting: Boolean): Int {
        ...
        try {
            ...
            while (reader.readLine().also { line = it } != null) {
                ...
                var priceType = io.refueler.merchant.core.model.PriceType.FIAT
                var priceSats = 0L
                ...
                if (values.size > 24 && values[24].isNotBlank()) {
                    try {
                        priceType = io.refueler.merchant.core.model.PriceType.valueOf(values[24])
                    } catch (e: Exception) {}
                }
                ...
                } else if (priceType == io.refueler.merchant.core.model.PriceType.SATS) {
                    // Fallback if priceSats is missing but priceType is SATS
                    priceSats = price.toLong()
                }
                
                if (priceType == io.refueler.merchant.core.model.PriceType.SATS) {
    ...
    fun exportItemsToCsv(outputStream: OutputStream): Boolean {
        try {
            ...
            for (item in items) {
                ...
                // Export price
                if (item.priceType == io.refueler.merchant.core.model.PriceType.FIAT) {
